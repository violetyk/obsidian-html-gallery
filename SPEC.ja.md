...
`manifest.json` の `isDesktopOnly` は `false`。「既定のアプリで開く」はデスクトップのみ有効にする（`Platform.isDesktopApp`）。

2026-09-07 に実機で確認した結果、モバイルでも HTML の iframe サムネイル、PDF の1ページ目描画、カード長押しの右クリックメニューはいずれも動作する。したがってデスクトップ限定にはしない。

ただし開き方が違う。モバイルには縦のリボンバーが無いため `addRibbonIcon` の項目はそのままでは見えず、ナビゲーションメニューのリボンボタンをタップして開くメニューの中に入る（設定 → 外観 の「Quick access ribbon item」で直接開くようにもできる）。コマンドパレットとフォルダ長押しのメニューはデスクトップと同じ。README にはこの違いを書く。
